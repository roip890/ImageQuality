...
class Data(object):
    ...
    def sample_data(self, samples, experts=Experts.All, experiment=Experiment.Random, distributions=None, accuracy_function=None):
        ...
        # return calculations
        return accuracy_function(data), self.group_data(data), samples
    ...
    @staticmethod
    def calculate_weighted_most_accuracy(data):
        # beginners
        beginners_ones_len = len([value[1] for value in data if value[1] == 1 and value[0] == Experts.Beginner.value])
        beginners_zeros_len = len([value[1] for value in data if value[1] == 0 and value[0] == Experts.Beginner.value])
        ...
        # ones
        ones_len = (beginners_ones_len * Experts.expert_price(Experts.Beginner)) + (mediums_ones_len * Experts.expert_price(Experts.Medium)) + (experts_ones_len * Experts.expert_price(Experts.Expert))
        # zeros
        zeros_len = (beginners_zeros_len * Experts.expert_price(Experts.Beginner)) + (mediums_zeros_len * Experts.expert_price(Experts.Medium)) + (experts_zeros_len * Experts.expert_price(Experts.Expert))
    ...
    @staticmethod
    def calculate_most_accuracy_with_tie_breaker(tie_breaker):
        def calculate_most_accuracy_with_tie_breaker_helper(data):
            ones_len = len([value[1] for value in data if value[1] == 1])
            zeros_len = len([value[1] for value in data if value[1] == 0])
            if ones_len > zeros_len:
                return 1
        ...
        return calculate_most_accuracy_with_tie_breaker_helper
