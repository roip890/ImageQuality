...
class Data(object):
    ...
    def sample_data(self, samples, experiment, experts, distributions):
        ...
        if distributions == None:
            ...
            sample_indices = []
            beginner_samples = min(int(samples * distributions.get(Experts.Beginner, 0)), len(experiment_range) * len(users_range))
            sample_indices.extend(random.sample(
                [(i, j, k) for i in [Experts.Beginner.value] for j in users_range for k in experiment_range], k=beginner_samples))
            medium_samples = min(int(samples * distributions.get(Experts.Medium, 0)), len(experiment_range) * len(users_range))
            sample_indices.extend(random.sample(
                [(i, j, k) for i in [Experts.Medium.value] for j in users_range for k in experiment_range], k=medium_samples))
            expert_samples = min(int(samples * distributions.get(Experts.Expert, 0)), len(experiment_range) * len(users_range))
            sample_indices.extend(random.sample(
                [(i, j, k) for i in [Experts.Expert.value] for j in users_range for k in experiment_range], k=expert_samples))
        ...
        # return calculations
        return self.calculate_accuracy(data), self.group_data(data), samples
